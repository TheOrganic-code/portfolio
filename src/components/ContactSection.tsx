...
export function ContactSection() {
  ...
  const links = [
    ...
    { label: 'LinkedIn', href: 'https://linkedin.com/in/ayushpandey1801', icon: (
      <svg width={20} height={20} viewBox="0 0 24 24" fill="currentColor">
  ...
  return (
    <section
      ...
      id="contact"
      style={{
        position: 'relative', zIndex: 10, maxWidth: 1280, margin: '0 auto', padding: '100px 32px 80px',
    ...
    >
      <div style={{ textAlign: 'center', maxWidth: 640, margin: '0 auto 60px' }}>
        <h2 id="contact-title" style={{ fontSize: 'clamp(32px, 4vw, 44px)', lineHeight: 1.15, marginBottom: 16 }}>
          Interested in physical systems, ML infrastructure, or difficult technical problems?
        </h2>
        ...
        {links.map((link) => (
          <a
            ...
            style={{
              ...
              padding: '14px 24px', border: '1px solid rgba(141,168,168,0.2)',
              borderRadius: 8, color: 'var(--text-main)', fontSize: 13, fontWeight: 500,
              fontFamily: 'var(--font-sans)', textDecoration: 'none',
              background: 'rgba(18,18,18,0.6)', backdropFilter: 'blur(10px)',
              transition: 'all 0.25s ease',
            }}
            onMouseEnter={(e) => {
              e.currentTarget.style.borderColor = 'var(--accent-cyan)'
              e.currentTarget.style.background = 'rgba(113,229,223,0.08)'
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.borderColor = 'rgba(141,168,168,0.2)'
      ...
      <div style={{
        borderTop: '1px solid rgba(141,168,168,0.15)', paddingTop: 40,
        display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, flexWrap: 'wrap' }}>
          <span style={{
            fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 600, color: 'var(--text-main)',
          }}>
            Ayush Pandey
          </span>
          <span style={{
            width: 8, height: 8, borderRadius: '50%', background: 'var(--accent-cyan)',
            boxShadow: pulse ? '0 0 12px var(--accent-cyan), 0 0 24px var(--accent-cyan)' : '0 0 8px var(--accent-cyan)',
          ...
          <span style={{
            ...
            B.Tech. Mathematics & Computing · RGIPT · Class of 2029
          </span>
        </div>
      ...
      <style jsx>{`
        @keyframes pulse {
          ...
          50% { box-shadow: 0 0 20px var(--accent-cyan), 0 0 40px rgba(113,229,223,0.4); }
        }
      `}</style>
    </section>
  )
